...
import { editOrderAction, getGovernorateFeesAction } from "@/lib/actions";
...
import { editOrderSchema } from "@/validations/editOrderSchema";
...
export function EditOrderDialog({ order, variant }: INewOrderProps) {
  ...
  async function onSubmit(values: z.infer<typeof editOrderSchema>) {
    ...
    const { error, message } = await editOrderAction({
      orderId: order._id.toString(),
      body: editedData,
      variant: variant === "orders" ? "update" : "zammitUpdate",
    });
    setLoading(false);
    if (error) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          ...
          className={`text-accent w-auto h-auto bg-transparent hover:bg-accent hover:text-white disabled:cursor-not-allowed`}
        >
          <PencilIcon className="size-4 text-inherit" />
        </Button>
      </DialogTrigger>
      <DialogContent className="lg:max-w-5xl overflow-scroll">
        <DialogHeader>
